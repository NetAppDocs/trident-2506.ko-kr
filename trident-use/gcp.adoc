...
Trident는 다음 두 가지 방법 중 하나로 Cloud Volumes Service 볼륨을 생성할 수 있습니다link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["서비스 유형"^]:

* *CVS-Performance*: 기본 Trident 서비스 유형입니다. 이 성능 최적화 서비스 유형은 성능을 중시하는 프로덕션 워크로드에 가장 적합합니다. CVS-Performance 서비스 유형은 최소 100GiB 크기의 볼륨을 지원하는 하드웨어 옵션입니다. 다음 옵션 중 하나를 선택할 수 있습니다 link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["세 가지 서비스 수준"^]:
+
** `standard`
** `premium`
** `extreme`
...
* *CVS*: CVS 서비스 유형은 제한적이거나 중간 수준의 성능으로 높은 영역 가용성을 제공합니다. CVS 서비스 유형은 스토리지 풀을 사용하여 1GiB 크기의 볼륨도 지원하는 소프트웨어 옵션입니다. 스토리지 풀은 최대 50개의 볼륨을 포함할 수 있으며, 모든 볼륨은 풀의 용량과 성능을 공유합니다. 다음 중 하나를 선택할 수 있습니다 link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["두 가지 서비스 수준"^]:
+
** `standardsw`
...
.필요한 것
 https://www.netapp.com/google-cloud/netapp-volumes/["Google Cloud용 Cloud Volumes Service"^] 백엔드를 구성하고 사용하려면 다음이 필요합니다.
