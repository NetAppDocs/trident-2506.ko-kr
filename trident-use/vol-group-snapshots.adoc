...
.단계
* VolumeGroupSnapshot을 생성하기 전에 VolumeGroupSnapshotClass를 생성하세요.  자세한 내용은 다음을 참조하세요.link:../trident-reference/objects.html#kubernetes-volumegroupsnapshotclass-objects["볼륨 그룹 스냅샷 클래스"] .
+
[source, yaml]
----
...
자세한 내용은 다음을 참조하세요. link:../trident-use/vol-snapshots.html#in-place-volume-restoration-from-a-snapshot["스냅샷에서 제자리 볼륨 복원"].
