...
[role="lead"]
Red Hat OpenShift를 사용하는 경우 Red Hat 인증 운영자를 사용하여 NetApp Trident 설치할 수 있습니다.  이 절차를 사용하여 Red Hat OpenShift Container Platform에서 Trident 설치하세요.
...
== Trident 운영자를 찾아 설치하세요

.단계
. OpenShift OperatorHub로 이동하여 NetApp Trident 를 검색합니다.
+
image::../media/openshift-operator-01.png[Trident 오퍼레이터]

. * NetApp Trident*를 클릭하여 설치 설정을 엽니다.
. 필요한 옵션을 선택하고 *설치*를 클릭하여 운영자 구성을 엽니다.
+
image::../media/openshift-operator-02.png[설치하다]
...
+
image::../media/openshift-operator-03.png[설치하다]
...
.단계
. 설치된 운영자 목록에서 Trident 운영자를 선택하세요.
. 연산자에서 모든 피연산자 인스턴스를 삭제할지 여부를 선택합니다.
+

WARNING: *이 연산자에서 모든 피연산자 인스턴스 삭제* 확인란을 선택하지 않으면 Trident 제거되지 않습니다.
