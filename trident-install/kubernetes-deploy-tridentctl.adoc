...
.시작하기 전에
설치를 시작하기 전에 Linux 호스트에 로그인하여 작동 중인 link:../trident-get-started/requirements.html["지원되는 Kubernetes 클러스터"]를 관리하고 있는지, 필요한 권한이 있는지 확인합니다.
...
NOTE: 설치 프로그램이 성공적으로 완료되지 않거나 `trident-controller-<generated id>` (`trident-csi-<generated id>`에 *실행 중* 상태가 표시되지 않으면 플랫폼이 설치되지 않은 것입니다.  `-d`를 사용하여 link:../troubleshooting.html#unsuccessful-trident-deployment-using-tridentctl["디버그 모드를 켜다"]하고 문제를 해결하세요.
...
[%collapsible]
====
강제 분리에 대한 자세한 내용은 link:kubernetes-customize-deploy.html["Trident 운영자 설치 사용자 정의"]을 참조하십시오.
