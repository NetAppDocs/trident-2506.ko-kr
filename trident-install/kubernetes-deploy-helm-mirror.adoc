...
summary: Helm을 사용하여 Trident 운영자를 배포하고 Trident 설치할 수 있습니다.  이 프로세스는 Trident 에 필요한 컨테이너 이미지가 개인 레지스트리에 저장된 시설에 적용됩니다. 
---
...
[role="lead"]
Helm을 사용하여 Trident 운영자를 배포하고 Trident 설치할 수 있습니다.  이 프로세스는 Trident 에 필요한 컨테이너 이미지가 개인 레지스트리에 저장된 시설에 적용됩니다.  개인 이미지 레지스트리가 없는 경우 다음을 사용하세요.link:kubernetes-deploy-helm.html["표준 배포 프로세스"] .
...
.시작하기 전에
link:../trident-install/kubernetes-deploy.html#before-you-deploy["배포 전제 조건"] 외에도 link:https://v3.helm.sh/["헬름 버전 3"^]이(가) 필요합니다.


IMPORTANT: 개인 저장소에 Trident 설치하는 경우 다음을 사용하는 경우 `imageRegistry` 저장소 위치를 지정하려면 스위치를 사용하세요. `/netapp/` 저장소 경로에.
...
----
. 사용 `helm install` 배포 및 이미지 레지스트리 위치에 대한 이름을 지정합니다.  당신의link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident 와 CSI 이미지"] 하나의 레지스트리나 여러 레지스트리에 위치할 수 있지만, 모든 CSI 이미지는 동일한 레지스트리에 위치해야 합니다.  예시에서, `100.2506.0` 설치하려는 Trident 버전입니다.
+
[role="tabbed-block"]
====
.하나의 레지스트리에 있는 이미지
...
----
nodePrep 값을 추가하려면 다음 명령을 실행하세요.
...
| `tridentControllerPluginNodeSelector` | 포드에 대한 추가 노드 선택기. 참조하다 link:../trident-get-started/architecture.html#understanding-controller-pods-and-node-pods["컨트롤러 포드와 노드 포드 이해"] 자세한 내용은. |  


| `tridentControllerPluginTolerations` | 포드에 대한 Kubernetes 허용을 재정의합니다. 참조하다 link:../trident-get-started/architecture.html#understanding-controller-pods-and-node-pods["컨트롤러 포드와 노드 포드 이해"] 자세한 내용은. |  


| `tridentNodePluginNodeSelector` | 포드에 대한 추가 노드 선택기. 참조하다 link:../trident-get-started/architecture.html#understanding-controller-pods-and-node-pods["컨트롤러 포드와 노드 포드 이해"] 자세한 내용은. |  


| `tridentNodePluginTolerations` | 포드에 대한 Kubernetes 허용을 재정의합니다. 참조하다 link:../trident-get-started/architecture.html#understanding-controller-pods-and-node-pods["컨트롤러 포드와 노드 포드 이해"] 자세한 내용은. |  
