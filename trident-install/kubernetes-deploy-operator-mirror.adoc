...
.시작하기 전에
Linux 호스트에 로그인하여 작동 중인지 확인하십시오.link:requirements.html["지원되는 Kubernetes 클러스터"^] 그리고 당신에게 필요한 특권이 있다는 것을 알려주세요.
...
이제 다음을 생성할 수 있습니다. `TridentOrchestrator` 그리고 Trident 설치하세요.  선택적으로 추가로 수행할 수 있습니다.link:kubernetes-customize-deploy.html["Trident 설치를 사용자 정의하세요"] 속성을 사용하여 `TridentOrchestrator` 투기.  다음 예는 Trident 와 CSI 이미지가 서로 다른 레지스트리에 있는 설치를 보여줍니다.
