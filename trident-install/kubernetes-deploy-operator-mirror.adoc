...
summary: Trident 운영자를 수동으로 배포하여 Trident 설치할 수 있습니다.  이 프로세스는 Trident 에 필요한 컨테이너 이미지가 개인 레지스트리에 저장된 시설에 적용됩니다. 
---
...
[role="lead"]
Trident 운영자를 수동으로 배포하여 Trident 설치할 수 있습니다.  이 프로세스는 Trident 에 필요한 컨테이너 이미지가 개인 레지스트리에 저장된 시설에 적용됩니다.  개인 이미지 레지스트리가 없는 경우 다음을 사용하세요.link:kubernetes-deploy-operator.html["표준 배포 프로세스"] .
...
link:../trident-install/kubernetes-deploy.html["설치 개요"]을(를) 검토하여 설치 사전 요구 사항을 충족했는지, 그리고 사용 환경에 맞는 올바른 설치 옵션을 선택했는지 확인하십시오.

.시작하기 전에
Linux 호스트에 로그인하여 정상적으로 작동하는 link:../trident-get-started/requirements.html["지원되는 Kubernetes 클러스터"]를 관리하고 있는지, 그리고 필요한 권한이 있는지 확인하십시오.
...
생성하다 `TridentOrchestrator` 사용자 정의 리소스 정의(CRD).  당신은 만들 것입니다 `TridentOrchestrator` 사용자 정의 리소스는 나중에.  적절한 CRD YAML 버전을 사용하세요. `deploy/crds` 생성하기 위해 `TridentOrchestrator` CRD:
...
~ 안에 `/deploy/operator.yaml` , 업데이트 `image: docker.io/netapp/trident-operator:25.06.0` 이미지 레지스트리의 위치를 반영합니다.  당신의link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident 와 CSI 이미지"] 하나의 레지스트리나 여러 레지스트리에 위치할 수 있지만, 모든 CSI 이미지는 동일한 레지스트리에 위치해야 합니다. 예를 들어:

* `image: <your-registry>/trident-operator:25.06.0`모든 이미지가 하나의 레지스트리에 있는 경우.
...
=== 5단계: 이미지 레지스트리 위치 업데이트 `TridentOrchestrator`

당신의link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident 와 CSI 이미지"] 하나의 레지스트리나 여러 레지스트리에 위치할 수 있지만, 모든 CSI 이미지는 동일한 레지스트리에 위치해야 합니다.  업데이트 `deploy/crds/tridentorchestrator_cr.yaml` 레지스트리 구성에 따라 추가 위치 사양을 추가합니다.
...
=== 6단계: 만들기 `TridentOrchestrator` 그리고 Trident 설치하세요

이제  `TridentOrchestrator`를 생성하고 Trident를 설치할 수 있습니다. 선택적으로, link:../trident-install/kubernetes-customize-deploy.html["Trident 설치를 사용자 정의하세요"]를  `TridentOrchestrator` 사양의 속성을 사용하여 추가로 구성할 수 있습니다. 다음 예시는 Trident와 CSI 이미지가 서로 다른 레지스트리에 위치한 설치를 보여줍니다.
