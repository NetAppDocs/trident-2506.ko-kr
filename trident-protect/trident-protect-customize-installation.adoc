...
summary: 귀하의 특정 요구 사항에 맞게 Trident Protect의 기본 설정을 사용자 정의하세요. 
---
...
.단계
. 라는 이름의 파일을 만듭니다. `resourceLimits.yaml` .
. 사용자 환경의 요구 사항에 따라 Trident Protect 컨테이너에 대한 리소스 제한 옵션으로 파일을 채웁니다.
+
...
jobResources:
  ...
  kopiaVolumeRestore:
    ...
    requests:
      ...
      ephemeralStorage: ""
----
. 값을 적용합니다 `resourceLimits.yaml` 파일:
+
[source, console]
----
...
.단계
. 라는 이름의 파일을 만듭니다. `sccconfig.yaml` .
...
| autoSupport.proxy | 끈 | NetApp AutoSupport 연결을 위한 프록시 URL을 구성합니다. 이를 사용하면 프록시 서버를 통해 지원 번들 업로드를 라우팅할 수 있습니다. 예: `http://my.proxy.url` . 
...
| restoreSkipNamespaceAnnotations | 끈 | 백업 및 복원 작업에서 제외할 네임스페이스 주석의 쉼표로 구분된 목록입니다. 주석을 기준으로 네임스페이스를 필터링할 수 있습니다. 


| restoreSkipNamespaceLabels | 끈 | 백업 및 복원 작업에서 제외할 네임스페이스 레이블의 쉼표로 구분된 목록입니다. 라벨을 기준으로 네임스페이스를 필터링할 수 있습니다. 
|===
...
.단계
. 라는 이름의 파일을 만듭니다. `nodeSelectorConfig.yaml` .
...
----
. 값을 적용합니다 `nodeSelectorConfig.yaml` 파일:
+
[source, console]
----
